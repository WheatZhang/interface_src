...
#include "IpDenseVector.hpp"  // zhangduo added
...
// zhangduo added
bool OrigIpoptNLP::InitializeHomotopyParameters(
    SmartPtr<Vector>& current_homotopy_target
)
{
   // Index n_homo_var = 2;
   // SmartPtr<DenseVectorSpace> d_curr_t_dest_space = new DenseVectorSpace(n_homo_var);
   // curr_t_dest_space_ = GetRawPtr(d_curr_t_dest_space);
   // current_homotopy_target = curr_t_dest_space_->MakeNew();

   bool retValue = nlp_->GetHomotopySpaces(curr_t_dest_space_, p_homo_space_, t_ori_space_, t_dest_space_);
   ...
   SmartPtr<Vector> t_Ori;
   SmartPtr<Vector> t_Dest;
   SmartPtr<Matrix> P_homo;

   t_Ori = t_dest_space_->MakeNew();
   t_Dest = t_dest_space_->MakeNew();
   P_homo = p_homo_space_->MakeNew();

   retValue = nlp_->GetHomotopyInformation(*t_Ori, *t_Dest, *P_homo);
   if( !retValue )
   {
      return false;
   }

   t_Ori->Print(*jnlst_, J_MOREVECTOR, J_INITIALIZATION,
               "original t_Ori unscaled");
   t_Dest->Print(*jnlst_, J_MOREVECTOR, J_INITIALIZATION,
               "original t_Dest unscaled");

   t_Ori_ = ConstPtr(t_Ori);
   t_Dest_ = ConstPtr(t_Dest);
   P_homo_ = ConstPtr(P_homo);

   t_Ori_ = NLP_scaling()->apply_vector_scaling_x_LU(*P_homo, t_Ori_, *x_space_);
   t_Dest_ = NLP_scaling()->apply_vector_scaling_x_LU(*P_homo, t_Dest_, *x_space_);

   t_Ori_->Print(*jnlst_, J_MOREVECTOR, J_INITIALIZATION,
               "modified t_Ori scaled");
   t_Dest_->Print(*jnlst_, J_MOREVECTOR, J_INITIALIZATION,
               "modified t_Dest scaled");
