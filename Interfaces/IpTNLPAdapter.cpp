...
bool TNLPAdapter::GetHomotopySpaces(
   ...
   SmartPtr<const VectorSpace>&    t_ori_space,
   SmartPtr<const VectorSpace>&    t_dest_space
)
{
   Index n_full_t;
   bool retval;
   ...
   SmartPtr<DenseVectorSpace> dt_ori_space = new DenseVectorSpace(n_t_);
   t_ori_space_ = GetRawPtr(dt_ori_space);
   SmartPtr<DenseVectorSpace> dt_dest_space = new DenseVectorSpace(n_t_);
   t_dest_space_ = GetRawPtr(dt_dest_space);
   SmartPtr<DenseVectorSpace> dcurr_t_dest_space = new DenseVectorSpace(n_t_);
   curr_t_dest_space_ = GetRawPtr(dcurr_t_dest_space);
   P_t_x_space_ = new ExpansionMatrixSpace(n_full_x_-n_x_fixed_, n_t_, x_t_map);
   p_homo_space_ = GetRawPtr(P_t_x_space_);

   delete[] x_t_map;
   x_t_map = NULL;

   // Assign the spaces to the returned pointers
   curr_t_dest_space = curr_t_dest_space_;
   p_homo_space = p_homo_space_;
   t_ori_space = t_ori_space_;
   t_dest_space = t_dest_space_;
   return true;
}
...
)
{
   ...
   if( IsValid(P_x_full_x_) )
   {
      ...
      {
         const Index& ipopt_idx = em_P_homo->ExpandedPosIndices()[i];
         const Index& full_idx = P_x_full_x_->ExpandedPosIndices()[ipopt_idx];
         const Number& homo_origin = t_ori[full_idx];
         values[i] = homo_origin;
      }
   }
   else
   {
      ...
      {
         const Index& ipopt_idx = em_P_homo->ExpandedPosIndices()[i];
         const Number& homo_origin = t_ori[ipopt_idx];
         values[i] = homo_origin;
      }
   }
   ...
   if( IsValid(P_x_full_x_) )
   {
      for( Index i = 0; i < P_homo.NCols(); i++ )
      {
         const Index& ipopt_idx = em_P_homo->ExpandedPosIndices()[i];
         const Index& full_idx = P_x_full_x_->ExpandedPosIndices()[ipopt_idx];
         const Number& homo_destination = t_dest[full_idx];
         values[i] = homo_destination;
      }
   }
   else
   {
      ...
      {
         const Index& ipopt_idx = em_P_homo->ExpandedPosIndices()[i];
         const Number& homo_destination = t_dest[ipopt_idx];
         values[i] = homo_destination;
      }
   }
