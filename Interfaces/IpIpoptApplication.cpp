...
)
{
   try
   {
      ...
      else
      {
         ...
         // zhangduo added
         // Open user defined output file if required
         options_->GetStringValue("homotopy_output_file", output_filename, "");
         if( output_filename != "" )
         {
            EJournalLevel user_defined_file_print_level;
            if( options_->GetIntegerValue("homotopy_print_level", ivalue, "") )
            {
               user_defined_file_print_level = (EJournalLevel) ivalue;
            }
            else
            {
               user_defined_file_print_level = print_level;
            }
            bool openend = OpenHomotopyOutputFile(output_filename, user_defined_file_print_level);
            if( !openend )
            {
               jnlst_->Printf(J_ERROR, J_INITIALIZATION, "Error opening output file \"%s\"\n", output_filename.c_str());
               return Invalid_Option;
